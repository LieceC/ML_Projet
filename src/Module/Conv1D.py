...
class Conv1D(Module):
    ...
    def backward_update_gradient(self, input, delta):
        ...
        a,_ = np.mgrid[0:input.shape[1]-self._k_size:(self._stride+1), 0:self._k_size] + np.arange(self._k_size)
        a = a.flatten()
    ...
    def backward_delta(self, input, delta):
        z = zip(range(0, input.shape[1], self._stride + 1), \
        range(self._k_size, input.shape[1], self._stride + 1))
        ...
        for i, (begin, end) in enumerate(z):
            ...
            res[:,begin:end] += d.reshape(-1, self._k_size, self._chan_in)   # remet les entrées comme précédemment        
        return res
